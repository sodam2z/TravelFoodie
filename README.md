...
> **Status**: Not implemented. No STT/TTS integration exists in the codebase.
...
### 6. 홈 위젯 (Home Widget) ⚠️
> **Status**: Basic widget structure exists but database integration incomplete. Currently shows hardcoded sample data.

- 다음 예정된 여행 정보 실시간 표시 ❌
- 여행까지 D-day 카운트다운 ⚠️ (hardcoded)
- 명소 및 맛집 개수 표시 ⚠️ (hardcoded)
...
### 8. 여행 계획 관리 (Trip Management) ⚠️
- 긴 터치(Long Press) → 수정/삭제 메뉴 팝업 ✅
- 진동 피드백으로 긴 터치 감지 ❌
- 스와이프로 계획 삭제 가능 ❌

### 9. 흔들기로 랜덤 맛집 추천 (Shake to Recommend) ⚠️
> **Status**: ShakeDetector class exists but not integrated into any UI. GPS geofencing and Lottie animations not implemented.

- 가속도센서로 폰 흔들면 자동 감지 ⚠️ (code exists but not used)
...
### 11. 보드/채팅 (Board/Chat) ❌
> **Status**: Module structure exists but completely unimplemented. No source code.

- Firebase Realtime Database로 팀원 간 실시간 채팅
- 이미지 첨부 기능 (Firebase Storage)
- @mention 기능으로 FCM 푸시 알림

### 12. OCR 영수증 스캔 (Receipt OCR) ❌
> **Status**: Not implemented. No ML Kit integration exists in the codebase.
...
:core:sensors           # Sensor handling (accelerometer) ⚠️ (code exists but not integrated)
:feature:trip           # Trip management feature ✅
:feature:attraction     # Attraction recommendations ✅
:feature:restaurant     # Restaurant listings ✅
:feature:voice          # Voice commands (STT/TTS) ❌ (module doesn't exist)
:feature:widget         # Home screen widget ⚠️ (partial implementation)
:feature:board          # Board/chat feature (optional) ❌ (empty module, no source code)
```
